...
import com.eliabdiel.springboot.bankingportal.bankingportal.exception.PasswordResetException;
import com.eliabdiel.springboot.bankingportal.bankingportal.exception.UnauthorizedException;
import com.eliabdiel.springboot.bankingportal.bankingportal.exception.UserInvalidException;
import com.eliabdiel.springboot.bankingportal.bankingportal.mapper.UserMapper;
...
import com.eliabdiel.springboot.bankingportal.bankingportal.service.EmailService;
import com.eliabdiel.springboot.bankingportal.bankingportal.service.GeolocationService;
...
import com.eliabdiel.springboot.bankingportal.bankingportal.util.JsonUtil;
import com.eliabdiel.springboot.bankingportal.bankingportal.util.LoggedinUser;
import com.eliabdiel.springboot.bankingportal.bankingportal.util.ValidationUtil;
...
public class UserServiceImpl implements UserService {
    ...
    public User getUserByAccountNumber(String accountNumber) {
        ...
        return userRepository.findByAccountAccountNumber(accountNumber).orElseThrow(
    ...
    private ResponseEntity<String> sendOtpEmail(User user, String otp) {
        val emailSendingFuture = otpService.sendOTPByEmail(user.getEmail(), user.getName(), user.getAccount().getAccountNumber(), otp);
        ResponseEntity<String> successResponse = ResponseEntity.ok(String.format(ApiMessages.OTP_SENT_SUCCESS.getMessage(), user.getEmail()));
        ResponseEntity<String> failureResponse = ResponseEntity.internalServerError().body(String.format(ApiMessages.OTP_SENT_FAILURE.getMessage(), user.getEmail()));
